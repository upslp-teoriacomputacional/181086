	...
	if($_[0] =~ m/$digito/){	
		...
		#therefore i just put that in to match instead of the variable.
			...
			if($char eq $Fin){
				...
				#i found out that we needed to implement a second else returning something other than 2 for this to work properly, otherwise strings like 3+e would be valid, and they are not.
				return 1;
			}
		}
	}
